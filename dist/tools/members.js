 ...
 */
export async function resolveAssignee(client, projectId, assignee) {
    ...
    if (emailMatch.length === 1)
        return emailMatch[0].user_id;
    // Try display name match (case-insensitive, partial)
    const nameMatch = members.filter(m => m.display_name?.toLowerCase().includes(query));
    if (nameMatch.length === 1)
        return nameMatch[0].user_id;
    if (nameMatch.length > 1) {
