export const queryTasksTool = {
    name: 'query_tasks',
    description: 'Search and filter tasks with rich criteria. All filters optional, combined with AND logic. Use for targeted queries; use list_tasks for simple unfiltered listing.',
    inputSchema: {
        type: 'object',
        properties: {
...
export async function queryTasks(client, projectId, args) {
    let query = client
        .from('tasks')
        .select('id, title, status, priority, task_number, assignee_id, epic_id, description, field_values, archived, due_date, created_at, updated_at, task_labels(labels(id, name, color))')
    ...
    if (args.epic_id)
        query = query.eq('epic_id', args.epic_id);
    if (args.cycle_id)
        query = query.eq('cycle_id', args.cycle_id);
    if (args.milestone_id)
        query = query.eq('milestone_id', args.milestone_id);
    if (args.priority)
        query = query.eq('priority', args.priority);
    if (args.due_date_from)
        query = query.gte('due_date', args.due_date_from);
    if (args.due_date_to)
        query = query.lte('due_date', args.due_date_to);
    if (args.stale_days) {
    ...
    if (args.label_ids && args.label_ids.length > 0) {
        enriched = enriched.filter((t) => args.label_ids.every((lid) => t.labels.some((l) => l.id === lid)));
    }
    return enriched;
}
//# sourceMappingURL=query-tasks.js.map