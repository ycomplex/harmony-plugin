...
export async function listCycles(client, projectId, args) {
    ...
    if (args.status)
        return withStatus.filter(c => c.derived_status === args.status);
    return withStatus;
}
...
export async function createCycle(client, projectId, userId, args) {
    ...
    if (existing && existing.length > 0)
        throw new Error('Cycles already exist for this project. Subsequent cycles are auto-created.');
...
export async function updateCycle(client, projectId, args) {
    const updates = {};
    if (args.name)
