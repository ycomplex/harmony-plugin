...
export async function listTasks(client, projectId, args) {
...
export async function getTask(client, projectId, args) {
...
export async function createTask(client, projectId, userId, args) {
...
export async function updateTask(client, projectId, args) {
    const resolvedId = await resolveTaskId(client, projectId, args.task_id);
    ...
    if (label_ids !== undefined) {
        ...
        if (label_ids.length > 0) {
            await client.from('task_labels').insert(label_ids.map((id) => ({ task_id: resolvedId, label_id: id })));
        }
    }
    return taskData;
}
...
export async function bulkCreateTasks(client, projectId, userId, args) {
    ...
    for (const t of allTasks ?? []) {
        if (!(t.status in maxPositions))
            maxPositions[t.status] = t.position;
    }
    ...
    await client.from('activity_events').insert((data ?? []).map((task) => ({
        task_id: task.id,
