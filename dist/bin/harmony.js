...
async function listSubtasks(client, projectId, args) {
  ...
  while (frontier.length > 0 && (depth === -1 || level < depth)) {
    const { data, error } = await client.from("tasks").select("id, parent_task_id, task_number, title, status, project_id, archived, created_at").in("parent_task_id", frontier).order("created_at", { ascending: true });
    if (error) throw error;
    const rows = data ?? [];
