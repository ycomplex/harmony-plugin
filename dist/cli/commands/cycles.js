...
export function registerCycleCommands(program) {
    ...
    cycles.command('list')
        ...
        .action(async (opts) => {
        await runCommand(program.opts(), async (ctx) => listCycles(ctx.client, ctx.projectId, { status: opts.status }), (data) => formatTable(data, [
    ...
    cycles.command('create')
        ...
        .action(async (opts) => {
        await runCommand(program.opts(), async (ctx) => createCycle(ctx.client, ctx.projectId, ctx.userId, {
    ...
    cycles.command('update')
        ...
        await runCommand(program.opts(), async (ctx) => updateCycle(ctx.client, ctx.projectId, {
            cycle_id: id,
            name: opts.name,
            end_date: opts.end,
        }), (cycle) => formatDetail([
