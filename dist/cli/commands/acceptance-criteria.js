...
export function registerAcceptanceCriteriaCommands(program) {
    ...
    ac.command('list')
        ...
        .action(async (taskId) => {
        await runCommand(program.opts(), async (ctx) => listAcceptanceCriteria(ctx.client, ctx.projectId, { task_id: taskId }), (data) => formatTable(data, [
    ...
    ac.command('add')
        ...
        await runCommand(program.opts(), async (ctx) => manageAcceptanceCriteria(ctx.client, ctx.projectId, ctx.userId, {
            task_id: taskId,
