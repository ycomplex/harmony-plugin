...
export function registerSubtaskCommands(program) {
    ...
    subtasks.command('list')
        ...
        .action(async (taskId) => {
        await runCommand(program.opts(), async (ctx) => listSubtasks(ctx.client, ctx.projectId, { task_id: taskId }), (data) => formatTable(data, [
    ...
    subtasks.command('add')
        ...
        await runCommand(program.opts(), async (ctx) => manageSubtasks(ctx.client, ctx.projectId, ctx.userId, {
            task_id: taskId,
