...
export function registerMilestoneCommands(program: Command): void {
  ...
  milestones.command('list')
    ...
    .action(async (opts) => {
      ...
      await runCommand(program.opts(), async (ctx) =>
        listMilestones(ctx.client, ctx.projectId, { status }),
  ...
  milestones.command('create')
    ...
    .action(async (opts) => {
      await runCommand(program.opts(), async (ctx) =>
        createMilestone(ctx.client, ctx.projectId, ctx.userId, {
          name: opts.name,
  ...
  milestones.command('update')
    ...
    .action(async (id, opts) => {
      await runCommand(program.opts(), async (ctx) =>
        updateMilestone(ctx.client, ctx.projectId, {
          milestone_id: id,
          name: opts.name,
          description: opts.description,
        }),
  ...
  milestones.command('ship')
    ...
    .action(async (id) => {
      await runCommand(program.opts(), async (ctx) =>
        shipMilestone(ctx.client, ctx.projectId, { milestone_id: id }),
        (result) => [
