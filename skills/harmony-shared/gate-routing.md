...
For `clarification-draft`, `decomposition-proposal`, `plan-draft`, and `design-decision-draft` — **B-904:
on EVERY design sub-track (product, technical, AND ux-ui alike), not product only** (the earlier
product-only restriction, discriminated by `decision_ref->>'type' = 'product-design'` at the DB, was
dropped by B-904 — all three sub-tracks now derive a `knowledge_entry_content` payload item uniformly and
defer via the event; see `harmony-design-decide/SKILL.md`'s own B-904 note), `resolve_brief`'s accept no
longer applies the state advance immediately: it snapshots the brief's payload into a
`pending_acceptance_events` row and DEFERS the advance. Every OTHER gate (release, verify, deny/park,
...
- **Same session** (the common case): the owning skill does its own materialization exactly as before
  ...
  the plan checklist), then calls **`consume_pending_acceptance_event({ task_id })`** (B-1029 — NOT the
  commit-only `consume_acceptance_event`: since B-866/B-867 the SAME event's payload can also carry
  `gate_slot`/`knowledge_entry_content` items the skill's own materialization never touches, so the
  same-session path must run the apply step too, not just commit) — see each skill's accept step.
