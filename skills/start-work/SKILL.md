...
2. **Write the marker's embedded patch text to a local file** (e.g. `/tmp/<ticket>-workflow.patch`) and **attach it** via `mcp__harmony__attach_file({ task_id, file_path })` — the subagent has no MCP tools and could not attach it itself.
